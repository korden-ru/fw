...
class upload
{
	...
	function ImageResized ($width = 150, $height = "", $filename, $is_resized = true, $is_watermark = false, $trim = false)
	{
		...
		if( $format == 'gif' )
		{
			...
			// @passthru(sprintf('%s "%s" -sample %dx%d +profile "*" "%s"', false === strpos(gethostname(), '.korden.net') ? '/usr/bin/convert' : '/usr/local/bin/gm convert', $prop['tmp_name'], $width, $height, $filename));
		}
		else
		{
			if( $trim )
			{
				@passthru(sprintf('%s "%s" -quality %d -filter triangle -trim -resize %dx%d\> -gravity south -background None -extent %dx%d +repage "%s"', false === strpos(gethostname(), '.korden.net') ? '/usr/bin/convert' : '/usr/local/bin/gm convert', $prop['tmp_name'], 75, $width, $height, $width, $height, $filename));
			...
			{
				// @passthru(sprintf('%s -size %dx%d "%s" -quality %d -filter triangle -resize %dx%d\> +repage "%s"', false === strpos(gethostname(), '.korden.net') ? '/usr/bin/convert' : '/usr/local/bin/gm convert', $width, $height, $prop['tmp_name'], 75, $width, $height, $filename));
			}
		}
		...
		// else
		// {
		// 	@passthru(sprintf('%s -size %dx%d "%s" -quality %d -filter triangle %s -resize %dx%d> +repage "%s"', false === strpos(gethostname(), '.korden.net') ? '/usr/bin/convert' : '/usr/local/bin/gm convert', $width, $height, $prop['tmp_name'], 75, (($trim) ? '-trim' : ''), $width, $height, $filename));
		// 	printf('%s -size %dx%d "%s" -quality %d -filter triangle %s -resize %dx%d> +repage "%s"', false === strpos(gethostname(), '.korden.net') ? '/usr/bin/convert' : '/usr/local/bin/gm convert', $width, $height, $prop['tmp_name'], 75, (($trim) ? '-trim' : ''), $width, $height, $filename);
		// 	exit;
		// }
		// 
		// return true;
	...
	function CroppedImageResized($width=300, $height=300, $filename, $ext=false)
	{
		...
		{
			@passthru(sprintf('%s -size %dx%d "%s" -quality %d -filter triangle -resize %dx%d\^ -gravity Center -crop %dx%d+0+0 +repage "%s"', false === strpos(gethostname(), '.korden.net') ? '/usr/bin/convert' : '/usr/local/bin/gm convert', $width, $height, $prop['tmp_name'], 75, $width, $height, $width, $height, $filename));
		}
		// портрет
		else
		{
